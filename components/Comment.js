...
import { HeartIcon as SoldiHeart } from "@heroicons/react/solid";
...
export default function Comment({comment, commentId, originalPostId}) {
  ...
  useEffect(()=>{
    const likesData = onSnapshot(
      collection(
        db, "posts", originalPostId, "comments", commentId, "likes"
  ...
  return (
    <div className="flex p-3 cursor-pointer border-b border-gray-200 pl-20">
      ...
      <div className="flex-1">
        ...
        <div onClick={() => router.push(`comments/${originalPostId}`)} >
          {/* comment text */}
        ...
        <div className="flex justify-between text-gray-500 p-2">
          ...
          {
            hasLiked ? (
            <SoldiHeart onClick={likeComment} className="h-9 w-9 hoverEffect p-2 text-red-600 hover:bg-red-100" />
            ) : ( 
