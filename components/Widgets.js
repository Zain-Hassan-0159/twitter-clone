...
export default function Widgets({newsResults, randomUsersResults}) {
  ...
  const [UserNumRandom, setUserNumRandom] = useState(3);
  return (
    <div className="xl:max-w-[350px] xl:w-[100%] xl:mr-0 mr-8 hidden lg:inline ml-8 space-y-5">
        {/* Search Bar */}
        ...
        {/* Who to follow */}
        <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 ">
        <h4 className="font-bold text-xl px-4">Who to follow</h4>
        <AnimatePresence>
          {randomUsersResults.slice(0, UserNumRandom).map(randomUser => (
        ...
        <button onClick={()=> setUserNumRandom(UserNumRandom +3) } className="text-blue-300 pl-4 pb-3 hover:text-blue-400">Show More</button>
        </div>
    </div>
  )
}
